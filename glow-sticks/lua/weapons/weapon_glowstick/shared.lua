...
function SWEP:Initialize()
	...
	self.Primary.DefaultClip = GetConVar("sv_glowstickammo"):GetFloat()
end
...
function SWEP:Think()
	...
	self:SetColor(Color(self.Owner:GetInfo("cl_glowsticks_red"), self.Owner:GetInfo("cl_glowsticks_green"), self.Owner:GetInfo("cl_glowsticks_blue"), 255)) -- Paints world model in real time
	end
	if SERVER then
		if (GetConVar("sv_glowstickfullauto"):GetFloat() == 1) then
...
function SWEP:Deploy()
	...
	local glow = ents.Create("ent_glowstick_glow")
		...
		glow:SetColor(Color(self.Owner:GetInfo("cl_glowsticks_red"), self.Owner:GetInfo("cl_glowsticks_green"), self.Owner:GetInfo("cl_glowsticks_blue"), 255))
		end
...
function SWEP:PrimaryAttack()
	...
	self.Weapon:SetNextPrimaryFire(CurTime() + GetConVar("sv_glowstickdropdelay"):GetFloat())
	self:TakePrimaryAmmo(1)
		...
		self.Owner:SetAnimation( PLAYER_ATTACK1 )
			if SERVER then
					...
					ent:SetColor( Color( self.Owner:GetInfo("cl_glowsticks_red"), self.Owner:GetInfo("cl_glowsticks_green"), self.Owner:GetInfo("cl_glowsticks_blue"), 255) )
					end
					ent:Spawn()
					ent:Activate()
...
function SWEP:SecondaryAttack()
	...
	self.Weapon:SetNextSecondaryFire(CurTime() + GetConVar("sv_glowstickthrowdelay"):GetFloat())
	self:TakePrimaryAmmo(1)
		timer.Simple(0.5, function()
		self.Owner:SetAnimation( PLAYER_ATTACK1 )
			if SERVER then
					...
					self.Owner:AddCleanup( "glowsticks", ent)
				local phys = ent:GetPhysicsObject()
				phys:SetVelocity(self.Owner:GetAimVector() * GetConVar("sv_glowstickthrowforce"):GetFloat() * 50)
				phys:AddAngleVelocity(Vector(math.random(-1000,1000),math.random(-1000,1000),math.random(-1000,1000)))
			end
		end)
		timer.Simple(1, function()
...
function SWEP:PreDrawViewModel( vm )
	Material("models/glowstick/glow"):SetVector("$color2", Vector(self.Owner:GetInfo("cl_glowsticks_red") / 255, self.Owner:GetInfo("cl_glowsticks_green") / 255, self.Owner:GetInfo("cl_glowsticks_blue") / 255) )
end
